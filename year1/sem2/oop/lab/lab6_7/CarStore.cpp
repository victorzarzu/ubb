...
DynamicArray<Masina> CarStore::generalSort(function<bool(const Masina& m1, const Masina& m2)> rel) {
	auto sorted{ getAll() };
	bool change = true;
	while (change) {
		change = false;
...
DynamicArray<Masina> CarStore::generalFilter(function<bool(const Masina& m)> filter) {
...
DynamicArray<Masina> CarStore::filterByProducator(string producator) {
	return generalFilter([producator](const Masina& m) {
		return m.GetProducator() == producator;
		});
}

DynamicArray<Masina> CarStore::filterByTip(string tip) {
