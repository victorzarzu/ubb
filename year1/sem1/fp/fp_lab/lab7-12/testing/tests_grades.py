...
from sorting.bingosort import bingosort
...
class TestsGrades(unittest.TestCase):
  ...
  def testMergeSort(self):
    s1 = StudentDTO("ABC")
    s1.add_grade(Grade(1, 1, 1, 5))
    s2 = StudentDTO("ABC")
    s2.add_grade(Grade(1, 1, 1, 6))
    s3 = StudentDTO("Mark")
    s3.add_grade(Grade(1, 1, 1, 7))
    s4 = StudentDTO("Mike")
    s4.add_grade(Grade(1, 1, 1, 1))

    l = [s2, s4, s3, s1]
    l = mergesort(l)

    self.assertEqual(l[0].get_name(), "ABC")
    self.assertAlmostEqual(l[0].average(), 5)

    self.assertEqual(l[1].get_name(), "ABC")
    self.assertAlmostEqual(l[1].average(), 6)

    self.assertEqual(l[2].get_name(), "Mark")
    self.assertEqual(l[3].get_name(), "Mike")
  ...
  def testBingoSort(self):
    ...
    l = [5, 6, 1, 25, 14, 1, 100, 6]
    l = bingosort(l)
    self.assertEqual(l, [1, 1, 5, 6, 6, 14, 25, 100])
