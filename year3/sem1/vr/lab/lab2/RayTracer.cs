...
namespace rt
{
    class RayTracer
    {
        ...
        {
            var background = new Color(88, 92, 89, 0.1);
            ...
            for (var i = 0; i < width; i++)
            {
                for (var j = 0; j < height; j++)
                {
                    ...
                    // color.Red = Math.Clamp(color.Red, 0, 1);
                    // color.Green = Math.Clamp(color.Green, 0, 1);
                    // color.Blue = Math.Clamp(color.Blue, 0, 1);
                    // color.Alpha = Math.Clamp(color.Blue, 0, 1);
