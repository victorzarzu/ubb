...
class FiniteAutomata:
    ...
    def concatenate(self, af):
        ...
        return self.__class__(transitions = transitions, initState = self._initState, type = self._type)
...
class FiniteAutomataReader:
    ...
    def read(self):
        ...
        transitions, initState, type =  self._file_read_elements() 
        return FiniteAutomata(transitions = transitions, initState = initState, type = type)

    def _console_read_elements(self):
        type = AtomClass.type[(input("AF type: "))]
        ...
        return transitions, initState, type

    def _file_read_elements(self):
        with open(self._filename, "r") as file:
            type = AtomClass[file.readline()[:-1]]
...
class DetFiniteAutomataReader(FiniteAutomataReader):
    ...
    def read(self):
        if self._filename is None:
            transitions, initState, type = self._console_read_elements()
            return DetFiniteAutomata(transitions = transitions, initState = initState, type = type)
        transitions, initState, type =  self._file_read_elements() 
        return DetFiniteAutomata(transitions = transitions, initState = initState, type = type)
